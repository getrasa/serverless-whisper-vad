...
import threading
import time
from whisper_vad.src.vad import AbstractTranscription, TranscriptionConfig, get_audio_duration
...
class ParallelContext:
    ...
    def _start_auto_cleanup(self):
        ...
        self.cleanup_timer = threading.Timer(
            self.auto_cleanup_timeout_seconds, self._execute_cleanup)
        self.cleanup_timer.start()

        print("Started auto cleanup of pool in " +
              str(self.auto_cleanup_timeout_seconds) + " seconds")
...
class ParallelTranscriptionConfig(TranscriptionConfig):
    def __init__(self, device_id: str, override_timestamps, initial_segment_index, copy: TranscriptionConfig = None):
        super().__init__(copy.non_speech_strategy, copy.segment_padding_left, copy.segment_padding_right,
                         copy.max_silent_period, copy.max_merge_size, copy.max_prompt_window, initial_segment_index)
...
class ParallelTranscription(AbstractTranscription):
    ...
    def transcribe_parallel(self, transcription: AbstractTranscription, audio: str, whisperCallable: WhisperCallback, config: TranscriptionConfig,
        ...
        if (cpu_device_count > 1 and not transcription.is_transcribe_timestamps_fast()):
            merged = self._get_merged_timestamps_parallel(
                transcription, audio, config, total_duration, cpu_device_count, cpu_parallel_context)
        else:
            timestamp_segments = transcription.get_transcribe_timestamps(
                audio, config, 0, total_duration)
            merged = transcription.get_merged_timestamps(
                timestamp_segments, config, total_duration)
        ...
        for i in range(len(gpu_devices)):
            ...
            device_segment_list = list(
                merged_split[i]) if i < len(merged_split) else []
            device_id = gpu_devices[i]

            print("Device " + str(device_id) + " (index " + str(i) +
                  ") has " + str(len(device_segment_list)) + " segments")

            # Create a new config with the given device ID
            device_config = ParallelTranscriptionConfig(
                device_id, device_segment_list, segment_index, config)
        ...
        print("Parallel transcription took " +
              str(perf_end_gpu - perf_start_gpu) + " seconds")
    ...
    def _get_merged_timestamps_parallel(self, transcription: AbstractTranscription, audio: str, config: TranscriptionConfig, total_duration: float,
        ...
        chunk_size = max(total_duration / cpu_device_count,
                         self.MIN_CPU_CHUNK_SIZE_SECONDS)
        chunk_start = 0
        cpu_device_id = 0
        ...
        try:
            ...
            results = pool.starmap(
                transcription.get_transcribe_timestamps, parameters)
            ...
            merged = transcription.get_merged_timestamps(
                timestamps, config, total_duration)

            perf_end_time = time.perf_counter()
            print("Parallel VAD processing took {} seconds".format(
    ...
    def get_merged_timestamps(self,  timestamps: List[Dict[str, Any]], config: ParallelTranscriptionConfig, total_duration: float):
        ...
        if (config.override_timestamps is not None):
            print("Using override timestamps of size " +
                  str(len(config.override_timestamps)))
