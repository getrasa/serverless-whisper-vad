...
import torch
from whisper_vad.src.modelCache import ModelCache
from whisper_vad.src.source import get_audio_source_collection
from whisper_vad.src.vadParallel import ParallelContext, ParallelTranscription
...
from whisper_vad.src.utils import slugify, write_srt, write_vtt
from whisper_vad.src.vad import AbstractTranscription, NonSpeechStrategy, PeriodicTranscriptionConfig, TranscriptionConfig, VadPeriodicTranscription, VadSileroTranscription
...
class WhisperTranscriber:
    def __init__(self, selected_model: str = "medium", input_audio_max_duration: float = DEFAULT_INPUT_AUDIO_MAX_DURATION, vad_process_timeout: float = None,
        ...
        self.model = WhisperContainer(
            model_name=selected_model, cache=self.model_cache)
    ...
    def transcribe_webui_simple(self, languageName, urlData, multipleFiles, microphoneData, task, vad, vadMergeWindow, vadMaxMergeSize, vadPadding, vadPromptWindow):
        return self.transcribe_webui(languageName, urlData, multipleFiles, microphoneData, task, vad, vadMergeWindow, vadMaxMergeSize, vadPadding, vadPromptWindow)
    ...
    def transcribe_webui(self, languageName, urlData, multipleFiles, microphoneData, task, vad, vadMergeWindow, vadMaxMergeSize, vadPadding, vadPromptWindow, **decodeOptions: dict):
        try:
            ...
            try:
                ...
                for source in sources:
                    ...
                    source_download, source_text, source_vtt = self.write_result(
                        result, filePrefix, outputDirectory)
                    ...
                    if (len(sources) > 1):
                        ...
                        zipFilePrefix = slugify(
                            source_prefix + source.get_short_name(max_length=200), allow_unicode=True)

                        # File names in ZIP file can be longer
                        for source_download_file in source_download:
                            # Get file postfix (after last -)
                            filePostfix = os.path.basename(
                                source_download_file).split("-")[-1]
                ...
                if len(sources) > 1:
                    downloadAllPath = os.path.join(
                        downloadDirectory, "All_Output-" + datetime.now().strftime("%Y%m%d-%H%M%S") + ".zip")

                    with zipfile.ZipFile(downloadAllPath, 'w', zipfile.ZIP_DEFLATED) as zip:
                        for download_file in download:
                            # Get file name from lookup
                            zip_file_name = zip_file_lookup.get(
                                download_file, os.path.basename(download_file))
    ...
    def transcribe_file(self, model: WhisperContainer, audio_path: str, language: str, task: str = None, vad: str = None,
        ...
        whisperCallable = model.create_callback(
            language, task, initial_prompt, **decodeOptions)

        # The results
        if (vad == 'silero-vad'):
            # Silero VAD where non-speech gaps are transcribed
            process_gaps = self._create_silero_config(
                NonSpeechStrategy.CREATE_SEGMENT, vadMergeWindow, vadMaxMergeSize, vadPadding, vadPromptWindow)
            result = self.process_vad(
                audio_path, whisperCallable, self.vad_model, process_gaps)
        elif (vad == 'silero-vad-skip-gaps'):
            # Silero VAD where non-speech gaps are simply ignored
            skip_gaps = self._create_silero_config(
                NonSpeechStrategy.SKIP, vadMergeWindow, vadMaxMergeSize, vadPadding, vadPromptWindow)
            result = self.process_vad(
                audio_path, whisperCallable, self.vad_model, skip_gaps)
        elif (vad == 'silero-vad-expand-into-gaps'):
            # Use Silero VAD where speech-segments are expanded into non-speech gaps
            expand_gaps = self._create_silero_config(
                NonSpeechStrategy.EXPAND_SEGMENT, vadMergeWindow, vadMaxMergeSize, vadPadding, vadPromptWindow)
            result = self.process_vad(
                audio_path, whisperCallable, self.vad_model, expand_gaps)
        elif (vad == 'periodic-vad'):
            # Very simple VAD - mark every 5 minutes as speech. This makes it less likely that Whisper enters an infinite loop, but
            # it may create a break in the middle of a sentence, causing some artifacts.
            periodic_vad = VadPeriodicTranscription()
            period_config = PeriodicTranscriptionConfig(
                periodic_duration=vadMaxMergeSize, max_prompt_window=vadPromptWindow)
            result = self.process_vad(
                audio_path, whisperCallable, periodic_vad, period_config)
            ...
            if (self._has_parallel_devices()):
                ...
                periodic_vad = VadPeriodicTranscription()
                period_config = PeriodicTranscriptionConfig(
                    periodic_duration=math.inf, max_prompt_window=1)
...
def create_ui(input_audio_max_duration, share=False, server_name: str = None, server_port: int = 7860,
    ...
    ui = WhisperTranscriber(input_audio_max_duration, vad_process_timeout,
                            vad_cpu_cores, DELETE_UPLOADED_FILES, output_dir)
    ...
    def simple_inputs(): return [
        ...
        gr.Dropdown(choices=["none", "silero-vad", "silero-vad-skip-gaps",
                    "silero-vad-expand-into-gaps", "periodic-vad"], value=default_vad, label="VAD"),
    ...
    full_transcribe = gr.Interface(fn=ui.transcribe_webui_full, description=full_description, article=ui_article, inputs=[
        *simple_inputs(),
        ...
        gr.Number(label="Best Of - Non-zero temperature",
                  value=5, precision=0),
        ...
        gr.Text(
            label="Suppress Tokens - Comma-separated list of token IDs", value="-1"),
    ...
    demo = gr.TabbedInterface(
        [simple_transcribe, full_transcribe], tab_names=["Simple", "Full"])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input_audio_max_duration", type=int, default=DEFAULT_INPUT_AUDIO_MAX_DURATION,
    ...
    parser.add_argument("--server_port", type=int,
                        default=7860, help="The port to bind to.")
    parser.add_argument("--default_model_name", type=str, choices=WHISPER_MODELS,
                        default="medium", help="The default model name.")
    parser.add_argument("--default_vad", type=str,
                        default="silero-vad", help="The default VAD.")
    parser.add_argument("--vad_parallel_devices", type=str, default="",
                        help="A commma delimited list of CUDA devices to use for parallel processing. If None, disable parallel processing.")
    parser.add_argument("--vad_cpu_cores", type=int, default=1,
                        help="The number of CPU cores to use for VAD pre-processing.")
    parser.add_argument("--vad_process_timeout", type=float, default="1800",
    ...
    parser.add_argument("--auto_parallel", type=bool, default=False,
    ...
    parser.add_argument("--output_dir", "-o", type=str,
                        default=None, help="directory to save the outputs")
